...
export function AutoCampaignTab() {
  ...
  // recorder.status must be a dependency — after a previous call it is
  // "stopped", and without re-running on that change auto-record never
  // re-engages for the rest of the session.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleDisposition = async (dispId: string) => {
    // Several awaits run before the UI resets, so without this guard a
    // double-click double-fires endCall + updateLeadStatus.
    ...
    // The lead status is updated even when no call id came back, otherwise the
    // lead stays "in_progress" forever and gets re-dialed on every pass.
    if (nextCampaignLead) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const noCallRecord = !activeCallId;

    // 3. Reset UI and schedule auto-dial of the next lead
    autoDialNextRef.current = true;
    setCallStatus("idle"); setDuration(0); setCallNotes(""); setActiveCallId(null);
    setCallError(noCallRecord ? "The call never started, so no call record was saved — only the lead status was updated." : null);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <Select value={selectedNumber} onValueChange={setSelectedNumber}>
          <SelectTrigger className="h-9 w-40 bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm [&>span]:truncate">
        ...
        {!isRunning ? (
          <>
            <select value={selectedCampaignId} onChange={(e) => setSelectedCampaignId(e.target.value)}
              className="bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 text-gray-900 dark:text-white text-sm h-9">
            ...
            <div className="relative">
              <Button
                variant="outline"
                className={`border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 h-9 px-3 text-sm ${fieldMenuOpen ? "bg-blue-600 border-blue-500 text-white" : "bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
              ...
              {fieldMenuOpen && (
                <>
                  ...
                  <div className="absolute left-0 mt-1 z-20 w-60 max-h-72 overflow-auto bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl p-2 shadow-2xl">
                    <p className="text-xs font-semibold text-gray-900 dark:text-gray-100 px-2 py-1.5 sticky top-0 bg-gray-100 dark:bg-gray-800 border-b border-gray-300 dark:border-gray-700 mb-1">Lead fields to display</p>
                    {STANDARD_FIELDS.map((f) => (
                      <label key={f.key} className="flex items-center gap-2 px-2 py-1.5 text-sm text-gray-900 dark:text-gray-100 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg cursor-pointer">
      ...
      <Card className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
        <CardContent className="p-4">
          ...
          <Progress value={progressPercentage} className="h-3 bg-gray-100 dark:bg-gray-800" />
        </CardContent>
      </Card>
      ...
      {isRunning && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="text-card-foreground flex flex-col gap-6 rounded-xl border py-6 shadow-sm bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
            <div className="px-6 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-gray-300 dark:border-gray-700">
                ...
                <Badge className="bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400 shrink-0">Current</Badge>
              </div>
              {currentLead ? (
                  ...
                  {displayFields.map((fk) => {
                    ...
                    return (
                      <div key={fk}>
                        ...
                        {fk === "notes" ? (
                          <Textarea value={getLeadValue(fk)} onChange={(e) => setLeadValue(fk, e.target.value)} onBlur={() => saveLeadField(fk)} placeholder={f.label} className="bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-600 mt-0.5 text-sm min-h-[55px]" />
                        ) : (
                          <Input value={getLeadValue(fk)} onChange={(e) => setLeadValue(fk, e.target.value)} onBlur={() => saveLeadField(fk)} placeholder={f.label} className="bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-600 mt-0.5 text-sm h-8" />
                        )}
                      </div>
                    );
                  })}
            ...
            <div className="px-6 space-y-3">
                  ...
                  {nextCampaignLead ? (
                    <div className="space-y-3">
                      <div className="text-center pt-2">
                        <div className="w-14 h-14 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center mx-auto mb-2">
                          <Phone className="w-7 h-7 text-gray-500 dark:text-gray-400" />
                        </div>
                        <p className="text-gray-500 dark:text-gray-400 text-sm">Ready to call</p>
                      </div>
                      <Input value={currentLead?.phone || ""} readOnly className="bg-gray-50 dark:bg-gray-950 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-center text-lg font-mono cursor-default" />
                      <div className="grid grid-cols-3 gap-1.5">
                        {["1","2","3","4","5","6","7","8","9","*","0","#"].map((d) => (
                          <button key={d} className="h-12 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white font-medium text-lg hover:bg-gray-200 dark:hover:bg-gray-700 active:bg-gray-300 dark:active:bg-gray-600 transition-colors border border-gray-300/50 dark:border-gray-700/50">{d}</button>
                        ))}
                      </div>
                      {callError && <div className="text-sm rounded-lg px-3 py-2 bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-400 border border-red-500/20">{callError}</div>}
              ...
              {callStatus === "calling" && (
                <div className="text-center py-8 space-y-4">
                  ...
                  <div>
                    <p className="text-lg font-semibold text-gray-900 dark:text-white">Dialing…</p>
                    <p className="text-gray-500 dark:text-gray-400 font-mono mt-1">{currentLead?.phone}</p>
              ...
              {callStatus === "connected" && (
                <div className="space-y-3">
                  <div className="text-center">
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white font-mono mt-2">{formatDur(duration)}</p>
                    <Badge className="bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400 mt-1">On Call</Badge>
                    {selectedNumber && <p className="text-xs text-gray-500 mt-1 font-mono">{selectedNumber}</p>}
                  </div>
                  {callError && <div className="text-xs rounded-lg px-3 py-2 bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-400 border border-red-500/20">{callError}</div>}
                  ...
                  <div className="flex justify-center gap-2 flex-wrap">
                    <Button variant="outline" size="sm" className={`border-gray-300 dark:border-gray-700 ${isMuted ? "text-red-400 bg-red-500/10" : "text-gray-600 dark:text-gray-300"}`} onClick={() => setIsMuted(!isMuted)}>
                      {isMuted ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4" />}
                    </Button>
                    <Button variant="outline" size="sm" className={`border-gray-300 dark:border-gray-700 text-xs gap-1 ${autoRecord ? "text-red-400 bg-red-500/10 border-red-700" : "text-gray-600 dark:text-gray-300"}`} onClick={handleAutoRecordToggle}>
                      <Radio className="w-3.5 h-3.5" /> Auto
                    </Button>
                    {!autoRecord && (
                      <Button variant="outline" size="sm" className={`border-gray-300 dark:border-gray-700 ${recorder.isRecording ? "text-red-400 bg-red-500/10 border-red-700 animate-pulse" : "text-gray-600 dark:text-gray-300"}`} onClick={handleToggleRecording}>
                  ...
                  <div className="rounded-xl bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 p-3">
                    <p className="text-xs font-medium text-gray-500 dark:text-gray-400 text-center mb-2">Keypad — IVR navigation</p>
                    <div className="grid grid-cols-3 gap-1.5">
                      {["1","2","3","4","5","6","7","8","9","*","0","#"].map((d) => (
                        <button key={d} onClick={() => { if (webrtcOn) rtc.sendDTMF(d); }}
                          className="h-11 rounded-lg bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white font-semibold text-lg hover:bg-gray-300 dark:hover:bg-gray-600 active:bg-gray-400 dark:active:bg-gray-500 active:scale-95 transition-all border border-gray-300 dark:border-gray-600">{d}</button>
                      ))}
                    </div>
                  </div>
                  <Textarea value={callNotes} onChange={(e) => setCallNotes(e.target.value)} placeholder="Call notes…" className="bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white min-h-[55px]" />
              ...
              {callStatus === "disposition" && (
                <div className="space-y-3">
                  <div className="text-center py-1">
                    ...
                    <p className="text-base font-semibold text-gray-900 dark:text-white">Call Ended</p>
                    <p className="text-gray-500 dark:text-gray-400 text-sm font-mono">{formatDur(duration)}</p>
                    {callError && <p className="text-xs text-red-400 mt-1">{callError}</p>}
                  </div>
                  {recorder.audioUrl && (
                    <div className="bg-gray-100/60 dark:bg-gray-800/60 border border-gray-300 dark:border-gray-700 rounded-lg p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        <p className="text-xs font-medium text-gray-600 dark:text-gray-300 flex items-center gap-1.5"><Disc className="w-3.5 h-3.5 text-red-400" /> Recording ({formatDur(recordingDuration)})</p>
                  ...
                  <Textarea value={callNotes} onChange={(e) => setCallNotes(e.target.value)} placeholder="Call notes…" className="bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white min-h-[55px] text-sm" />
                  <div>
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">Select Call Result</p>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      {(dispositions as any[]).map((disp) => (
                        <Button key={disp.id} size="sm" onClick={() => handleDisposition(disp.id.toString())}
                          ...
                          className="bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-900 dark:text-white justify-start h-10 border border-gray-400 dark:border-gray-500 transition-colors">
          ...
          <Card className="text-card-foreground flex flex-col gap-6 rounded-xl border py-6 shadow-sm bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
            <div className="px-6 space-y-4">
              <div className="text-center pb-3 border-b border-gray-300 dark:border-gray-700">
                <Clock className="w-6 h-6 text-gray-500 mx-auto mb-1" />
                <p className="text-2xl font-bold text-gray-900 dark:text-white font-mono">{formatDur(duration)}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Current Call Duration</p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Campaign Metrics</h4>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm"><span className="text-gray-500 dark:text-gray-400">Total Leads</span><span className="text-gray-900 dark:text-white font-medium">{progressTotal}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-gray-500 dark:text-gray-400">Completed</span><span className="text-green-400 font-medium">{progressCompleted}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-gray-500 dark:text-gray-400">Pending</span><span className="text-amber-400 font-medium">{campaignProgress?.pending || 0}</span></div>
                </div>
              </div>
      ...
      {!isRunning && (
        <Card className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Auto Dialer Ready</h3>
