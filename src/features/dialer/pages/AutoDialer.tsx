...
import {
  ...
  Phone, PhoneCall, CheckCircle2, XCircle, Ban, User, Building,
  Briefcase, MapPin, Mail, BarChart3, Disc, Square, Download,
  Mic, MicOff, Pencil, Check, X, Hash, Save, Plus,
...
function AutoCampaignTab() {
  ...
  const startEditLead = () => {
    if (!currentLead) return;
    setLeadEdit({ firstName: currentLead.firstName || "", lastName: currentLead.lastName || "", phone: currentLead.phone || "", email: currentLead.email || "", companyName: currentLead.companyName || "", designation: currentLead.designation || "", city: currentLead.city || "", notes: currentLead.notes || "" });
    setIsEditingLead(true); setLeadSaveMsg(null);
  };
  const cancelEditLead = () => { setIsEditingLead(false); setLeadEdit({}); };
  const saveEditLead = async () => {
    if (!currentLead) return;
    try {
      await updateLeadMutation.mutateAsync({ id: currentLead.id, data: { firstName: leadEdit.firstName || undefined, lastName: leadEdit.lastName || undefined, phone: leadEdit.phone || undefined, email: leadEdit.email || undefined, companyName: leadEdit.companyName || undefined, designation: leadEdit.designation || undefined, city: leadEdit.city || undefined, notes: leadEdit.notes || undefined } });
      setLeadSaveMsg("Client details updated ✓"); setIsEditingLead(false); refetchNextLead();
      setTimeout(() => setLeadSaveMsg(null), 3000);
    } catch { setLeadSaveMsg("Failed to save changes"); }
  };
  ...
  const handleDisposition = async (dispId: string) => {
    ...
    recorder.resetRecording(); setSavedRecordingDataUrl(null); setRecordingDuration(0); setIsMuted(false); setIsEditingLead(false); setLeadEdit({});
  ...
  const fieldIcon = (key: string) => {
    if (key === "companyName") return <Building className="w-4 h-4 text-gray-500" />;
            ...
            <CardContent className="p-5 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-gray-800">
                <div className="flex items-center gap-3">
                  ...
                  {currentLead
                    ? <div><h3 className="text-base font-semibold text-white">{currentLead.firstName || ""} {currentLead.lastName || "Lead"}</h3><p className="text-xs text-gray-500">Lead ID: #{currentLead.id}</p></div>
                    : <div><h3 className="text-base font-semibold text-white">Searching...</h3><p className="text-xs text-gray-500">Finding next pending lead</p></div>}
                </div>
                <div className="flex items-center gap-1">
                  {currentLead && !isEditingLead && (
                    <>
                      <div className="relative">
                        <Button variant="outline" size="sm" className={`border-gray-700 text-gray-300 h-7 px-2 text-xs ${fieldMenuOpen ? "bg-blue-600 border-blue-500 text-white" : ""}`} onClick={() => setFieldMenuOpen((o) => !o)}>Fields ▾</Button>
                        {fieldMenuOpen && (
                          <>
                            <div className="fixed inset-0 z-10" onClick={() => setFieldMenuOpen(false)} />
                      ...
                      <Button variant="outline" size="sm" title="Edit client details" className="border-gray-700 text-gray-300 h-7 px-2" onClick={startEditLead}><Pencil className="w-3.5 h-3.5" /></Button>
                    </>
                  )}
              ...
              {leadSaveMsg && <p className={`text-xs text-center rounded-md py-1 ${leadSaveMsg.includes("Failed") ? "text-red-400 bg-red-500/10" : "text-green-400 bg-green-500/10"}`}>{leadSaveMsg}</p>}
              ...
              {currentLead && isEditingLead ? (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 pt-1">
                    <Button size="sm" className="flex-1 bg-green-600 hover:bg-green-700 h-7" onClick={saveEditLead} disabled={updateLeadMutation.isPending}><Check className="w-3.5 h-3.5 mr-1" /> Save</Button>
                    <Button size="sm" variant="outline" className="flex-1 border-gray-700 text-gray-300 h-7" onClick={cancelEditLead}><X className="w-3.5 h-3.5 mr-1" /> Cancel</Button>
                  </div>
                </div>
              ) : currentLead ? (
                <div className="space-y-2 text-sm">
                  ...
                  {!displayFields.includes("phone") && currentLead.phone && (
                    <div className="flex items-center gap-2 text-gray-300"><Phone className="w-4 h-4 text-gray-500" /> {currentLead.phone}</div>
                  )}
                  {currentLead.notes && displayFields.includes("notes") && (
                    <div className="pt-2 border-t border-gray-800"><p className="text-[10px] font-medium text-gray-500 uppercase mb-0.5">Notes</p><p className="text-xs text-gray-400 italic">{currentLead.notes}</p></div>
                  )}
                </div>
              ) : (
                <div className="text-center py-4 text-gray-500 text-sm">Waiting for next pending campaign lead...</div>
              )}
            </CardContent>
          </Card>
